...
void update2(int index,int l,int r,int val,int base){
    ...
    if(index<mid){
        ...
        seg2[base] = seg2[2*base]+seg2[2*base+1];
    }else{
...
for(int i=0;i<N;i++){
    sc1(a[i]);
    update1(i,0,N-1,a[i],1);
    update2(i,0,N-1,a[i],1);
    update3(i,0,N-1,a[i],1);
...
while(M--){
    ...
    if(c=='U'){
        a[x-1] = y;
        update1(x-1,0,N-1,y,1);
        update2(x-1,0,N-1,y,1);
        update3(x-1,0,N-1,y,1);
        update4(x-1,0,N-1,y,1);
    }
    else if(c=='M'){
        pf1(search1(x-1,y-1,0,N-1,1));br;
    }
    else if(c=='S'){
        pf1(search2(x-1,y-1,0,N-1,1));br;
    }
    else if(c=='I'){
        pf1(search3(x-1,y-1,0,N-1,1));br;
    }
    else{
        pf1(search4(x-1,y-1,0,N-1,1));br;
    }
}
return 0;
}
